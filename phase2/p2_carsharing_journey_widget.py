...
def counts_by_hexagon(df, resolution,latlong,filter_variable=None):    
    ...
    df_aggreg = df.groupby(by = ["hex_id","hex_no"]).agg({'vehicle_id':'count','object_data-fare':'mean'}).reset_index()
    print(len(df_aggreg))
...
def hexagons_dataframe_to_geojson(df_hex, file_output = None):
    ...
    for i,row in df_hex.iterrows():
        ...
        feature = Feature(geometry = row["geometry"] , id=row["hex_id"], properties = {"Hex_Count" : row["value"],"Hex_No":row["hex_no"],"Average_Fare":row['average_fare']})
...
def my_slider_handler():
    ...
    try:
        ...
        geo_source.geojson=json.dumps({"type":"FeatureCollection","features":[{"type":"Feature","id":"832830fffffffff","geometry":{"type":"Polygon","coordinates":[[[] for x in range(1)]]},"properties":{'Hex_Count': 1, 'Hex_No': '27','Average_Fare': 2.5}}]})
...
display_columns2=['Hex_Count','Hex_No','Average_Fare']
from bokeh.models import HoverTool
TOOLTIP2=HoverTool()
...
drs_start=DatePicker(title='Date',min_date=datetime.date(2020,3,15),max_date=datetime.date(2020, 6, 1),value=Min_date)
drs_start_hour = Select(title='Hour of the day',value='0', options=[str(x) for x in list(range(0,24))])

drs_end=DatePicker(title='Date',min_date=datetime.date(2020,3,15),max_date=datetime.date(2020, 6, 1),value=Max_date)
...
df_rentals=df.groupby('customer_id').agg({'object_data-fare':'sum','vehicle_id':'count'}).reset_index()
df_rentals.columns=['customer_id','total_fare','number_of_rentals']
data_table_Columns = [TableColumn(field=Ci, title=Ci) for Ci in df_rentals.columns] # bokeh columns
data_table_source=ColumnDataSource(df_rentals)
data_table = DataTable(columns=data_table_Columns, source=data_table_source,width=800) # bokeh table
...
data_table=Panel(child=column(row(data_table),row(download_button)),title='Data Table')


main_tab=Tabs(tabs=[base_widgets,visibility_widgets,customer_widgets,data_table])
...
def callback(event):
    ...
    df_rentals=dff.groupby('customer_id').agg({'object_data-fare':'sum','vehicle_id':'count'}).reset_index()
    df_rentals.columns=['customer_id','total_fare','number_of_rentals'] # bokeh columns
    dictionary3={}
    for col_name in df_rentals.columns:
      dictionary3[col_name]=df_rentals[col_name]
